...
public class TestRepositoryUser {
    ...
    private static void prepareRepositories(){
        REPOSITORY_NOT_GENERATE_ID = new RepositoryUser<>(CREATE_DB_INTEGER_CONNECTION, GenerateIdType.NONE);
        REPOSITORY_GENERATE_NUMBER_ID = new RepositoryUser<>(CREATE_DB_INTEGER_CONNECTION, GenerateIdType.INT);
        REPOSITORY_GENERATE_UUID_ID = new RepositoryUser<>(CREATE_DB_UUID_CONNECTION, GenerateIdType.UUID);
    ...
    public void test020IsDeleted(){
        boolean result = REPOSITORY_GENERATE_NUMBER_ID.isDeleted((Integer)data[2].getId());
        Assert.assertFalse(result);
    }

    @Test
    public void test021IsBlocked(){
        boolean result = REPOSITORY_GENERATE_NUMBER_ID.isBlocked((Integer)data[2].getId());
        Assert.assertFalse(result);
    }

    @Test
    public void test022IsConfirmEmail(){
        boolean result = REPOSITORY_GENERATE_NUMBER_ID.isConfirmEmail((Integer)data[2].getId());
        Assert.assertFalse(result);
    }
