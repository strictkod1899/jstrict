...
public class TestRepositoryJWTToken {
    ...
    private static void prepareRepositories(){
        REPOSITORY_NOT_GENERATE_ID = new RepositoryJWTToken<>(CREATE_DB_INTEGER_CONNECTION, GenerateIdType.NONE);
        REPOSITORY_GENERATE_NUMBER_ID = new RepositoryJWTToken<>(CREATE_DB_INTEGER_CONNECTION, GenerateIdType.INT);
        REPOSITORY_GENERATE_UUID_ID = new RepositoryJWTToken<>(CREATE_DB_UUID_CONNECTION, GenerateIdType.UUID);
    ...
    public void test005ReadByUuid(){
        JWTToken model = REPOSITORY_GENERATE_UUID_ID.read((UUID)data[1].getId());
    ...
    private void convertUUIDModel(JWTToken model){
