...
public class TestRepositoryCity {
    ...
    private static void prepareRepositories(){
        REPOSITORY_NOT_GENERATE_ID = new RepositoryCity(CREATE_DB_INTEGER_CONNECTION, GenerateIdType.NONE);
        REPOSITORY_GENERATE_NUMBER_ID = new RepositoryCity(CREATE_DB_INTEGER_CONNECTION, GenerateIdType.INT);
        REPOSITORY_GENERATE_UUID_ID = new RepositoryCity<>(CREATE_DB_UUID_CONNECTION, GenerateIdType.UUID);
    ...
    private static void prepareData(){
        IRepositoryNamed<Integer, Country<Integer>> repositoryCountryNumberId = new RepositoryCountry<>(CREATE_DB_INTEGER_CONNECTION, GenerateIdType.NONE);
        IRepositoryNamed<UUID, Country<UUID>> repositoryCountryUuidId = new RepositoryCountry<>(CREATE_DB_UUID_CONNECTION, GenerateIdType.NONE);
    ...
    public void test004ReadByInteger(){
        ModelBase<Integer> model = REPOSITORY_GENERATE_NUMBER_ID.read((Integer)data[2].getId());
    ...
    public void test005ReadByUuid(){
        City model = REPOSITORY_GENERATE_UUID_ID.read((UUID)data[1].getId());
    ...
    private void convertUUIDModel(City model){
        model.setId(UUID.fromString(model.getId().toString()));
        model.setCountryId(UUID.fromString(model.getCountryId().toString()));
    }
}
