...
import static ru.strict.db.spring.runners.TestRunner.CREATE_DB_UUID_CONNECTION;

@FixMethodOrder(MethodSorters.NAME_ASCENDING)
public class TestRepositoryUserSecurity {

    private static UserSecurity[] data;

    private static IRepositoryUser<Long, UserSecurity<Long>> REPOSITORY_NOT_GENERATE_ID;
    private static IRepositoryUser<Long, UserSecurity<Long>> REPOSITORY_GENERATE_NUMBER_ID;
    private static IRepositoryUser<UUID, UserSecurity<UUID>> REPOSITORY_GENERATE_UUID_ID;
    ...
    private static void prepareRepositories(){
        REPOSITORY_NOT_GENERATE_ID = new RepositoryUserSecurity<>(CREATE_DB_INTEGER_CONNECTION, GenerateIdType.NONE, JDBCType.BIGINT);
        REPOSITORY_GENERATE_NUMBER_ID = new RepositoryUserSecurity<>(CREATE_DB_INTEGER_CONNECTION, GenerateIdType.LONG, JDBCType.BIGINT);
        REPOSITORY_GENERATE_UUID_ID = new RepositoryUserSecurity<>(CREATE_DB_UUID_CONNECTION, GenerateIdType.UUID, SqlType.UUID);
    ...
    private static void fillData(){
        data = new UserSecurity[]{
                ...
                new UserSecurity(USER1),
                new UserSecurity(USER2),
                new UserSecurity(USER3),
                new UserSecurity(USER1_UPDATED)
        };
    }
    ...
    public void test009ReadByName(){
        ModelBase model = REPOSITORY_GENERATE_NUMBER_ID.readByName(data[0].getUsername());
    ...
    public void test010ReadAllByName(){
        List list = REPOSITORY_GENERATE_NUMBER_ID.readAllByName(data[0].getUsername());
        Assert.assertEquals(1, list.size());
    ...
    public void test020IsDeleted(){
        boolean result = REPOSITORY_GENERATE_NUMBER_ID.isDeleted((Long)data[2].getId());
        Assert.assertFalse(result);
    }

    @Test
    public void test021IsBlocked(){
        boolean result = REPOSITORY_GENERATE_NUMBER_ID.isBlocked((Long)data[2].getId());
        Assert.assertFalse(result);
    }

    @Test
    public void test022IsConfirmEmail(){
        boolean result = REPOSITORY_GENERATE_NUMBER_ID.isConfirmEmail((Long)data[2].getId());
        Assert.assertFalse(result);
    }
}
