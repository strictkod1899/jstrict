...
public class SystemUtil {
    ...
    public static void openFile(String filePath) throws IOException {
        ...
        switch (SystemUtil.getOperatingSystemType()) {
            case WINDOWS:
                Runtime.getRuntime().exec(String.format("cmd /c %s", filePath));
                break;
            case LINUX:
                Runtime.getRuntime().exec(String.format("xdg-open %s", filePath));
                break;
            default:
                throw new UnsupportedOperationException("current OS not supported");
        }
    }
