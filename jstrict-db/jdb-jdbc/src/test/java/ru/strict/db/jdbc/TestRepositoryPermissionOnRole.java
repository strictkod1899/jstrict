...
import ru.strict.db.jdbc.data.Permission;
...
import ru.strict.db.jdbc.repositories.RepositoryPermissionOnRole;
...
public class TestRepositoryPermissionOnRole extends TestBase {

    private static PermissionOnRole[] data;

    private static IRepositoryPermissionOnRole<Long, Permission> REPOSITORY_NOT_GENERATE_ID;
    private static IRepositoryPermissionOnRole<Long, Permission> REPOSITORY_GENERATE_NUMBER_ID;
    ...
    private static void prepareRepositories(){
        REPOSITORY_NOT_GENERATE_ID = new RepositoryPermissionOnRole<>(CREATE_DB_INTEGER_CONNECTION, GenerateIdType.NONE, JDBCType.BIGINT);
        REPOSITORY_GENERATE_NUMBER_ID = new RepositoryPermissionOnRole<>(CREATE_DB_INTEGER_CONNECTION, GenerateIdType.LONG, JDBCType.BIGINT);
        REPOSITORY_GENERATE_UUID_ID = new RepositoryPermissionOnRole<>(CREATE_DB_UUID_CONNECTION, GenerateIdType.UUID, SqlType.UUID);
    ...
    private static void prepareData(){
        ...
        repositoriesForClearDb.add(repositoryRoleNumberId);
        repositoriesForClearDb.add(repositoryRoleUuidId);
    ...
    public void test020ReadByRoleId(){
        List<PermissionOnRole<Long, Permission>> list = REPOSITORY_GENERATE_NUMBER_ID.readByRoleId((Long)data[2].getRoleId());
        Assert.assertEquals(2, list.size());
        Assert.assertTrue(list.contains(data[2]));
