...
import ru.strict.db.jdbc.mappers.sql.SqlMapperBase;
...
public abstract class RepositoryJdbcBase
     ...
     */
    private SqlMapperBase<ID, T> sqlMapper;

    //<editor-fold defaultState="collapsed" desc="constructors">
    private void init(SqlMapperBase<ID, T> sqlMapper){
        if(sqlMapper == null){
     ...
     */
    protected RepositoryJdbcBase(DbTable table,
                       String[] columns,
                       ICreateConnection<Connection> connectionSource,
                       GenerateIdType generateIdType,
                       SQLType sqlIdType) {
    ...
    private String createSqlInsertFull(int parametersCount){
        StringBuilder sql = new StringBuilder(String.format("INSERT INTO %s (%s, ", getTable().getTableName(), getIdColumnName()));
        for(String columnName : getColumns()) {
            sql.append(columnName);
            sql.append(", ");
        }
    ...
    protected void setSqlMapper(SqlMapperBase<ID, T> sqlMapper){
        this.sqlMapper = sqlMapper;
    }

    public SqlMapperBase<ID, T> getSqlMapper() {
        return sqlMapper;
    }
    //</editor-fold>
